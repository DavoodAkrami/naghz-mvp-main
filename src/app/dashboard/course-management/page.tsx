...
import { toDbCorrectAnswer, fromDbCorrectAnswer, getPageDisplayText, swapArrayItems, swapObjKeys, renumberPagesInPlace, movePageUp, movePageDown, type PageFormData } from "@/utils/courseManagement";
...
export interface FullCourseFormData {
  slug: string;
  title: string;
  description: string;
  icon_name: string;
...
export default function CourseManagement() {
  ...
  const handleMovePageUp = (pageIndex: number) => {
    ...
    const { newPages, newOptionsByPage, newImagesByPage } = movePageUp(pages, pageIndex, optionsByPage, imagesByPage);
    setOptionsByPage(newOptionsByPage);
    setImagesByPage(newImagesByPage);
    setPages(newPages);
  };
  ...
  const handleMovePageDown = (pageIndex: number) => {
    ...
    const { newPages, newOptionsByPage, newImagesByPage } = movePageDown(pages, pageIndex, optionsByPage, imagesByPage);
    setOptionsByPage(newOptionsByPage);
    setImagesByPage(newImagesByPage);
    setPages(newPages);
  };
  ...
  return (
    <div className="p-6" dir="rtl">
      ...
      {/* Create Course Modal */}
      <Modal onOpen={showCreateModal} onClose={() => setShowCreateModal(false)} classname="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      ...
      {/* Edit Course Modal */}
      <Modal onOpen={showEditModal} onClose={() => setShowEditModal(false)} classname="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <Modal onOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} classname="max-w-md w-full">
      ...
      {/* Reorder Pages Modal */}
      <Modal onOpen={showReorderModal} onClose={() => setShowReorderModal(false)} classname="max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <Modal onOpen={showCreateFullCourseModal} onClose={() => setShowCreateFullCourseModal(false)}>
      ...
      <Modal onOpen={showEditFullCourseModal} onClose={() => setShowEditFullCourseModal(false)}>
      ...
      <Modal onOpen={showDeleteFullCourseModal} onClose={() => setShowDeleteFullCourseModal(false)} classname="max-w-md w-full">
